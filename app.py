...
def insta_prediction():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            result = 'Account not found. This account does not exist on INSTAGRAM'
        ...
        if(following < 100):
            ...
            result = 'Data is insufficient, followers : ', followers, 'following : ', following
        if(following > 100):
            if(ratio > 10):
                # return jsonify({'result':'Fake account'})
                result = 'Fake account, followers : ', followers, 'following : ', following
            else:
                # return jsonify({'result':'Valid account'})
                result = 'Valid account,  followers : ', followers, 'following : ', following
    return render_template('result.html', account_prediction_text=result)
